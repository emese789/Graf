...
void melysegimenet(graf*g,int csomopontok,int elekszama,int* szin,int* apa,int* elert,int* elhagy,int ido,int csomopont)
{
    ...
    for(int i=1; i<=elekszama; ++i)
    {
        ...
        if(g->elek[i].kezd==csomopont && szin[g->elek[i].veg]==2)
        {
            if(elert[g->elek[i].veg]>elert[csomopont])
            {
                printf("%i -> %i eloremutato el\n",csomopont,g->elek[i].veg);
            }
            else
            {
                printf("%i -> %i kereszt el\n",csomopont,g->elek[i].veg);
            }
        }
        ...
        if(g->elek[i].kezd==csomopont && szin[csomopont]==1)
        {
